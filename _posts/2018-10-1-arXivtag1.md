...
The categories I've chosen are: astro(nomy), cond(ensed matter), cs, hep(high-energy physics), math, physics, qbio (quantitative biology), qfin (quantitative finance), quant(um mechanics), stat(istics), others

```python
dictname = "../Data/dict/full_articleset.p"
    article_dic = pickle.load(open(dictname, "rb"))

    # keys that look like this oai:arXiv.org:adap-org/9806001 old version, wo do not use
...
```----2011----
soup cat loaded, size: 150728726 bytes
soup abs loaded, size: 150728726 bytes
